    ...
    function initMobileHeaderMenu() {
        ...
        if (actions) {
            actions.querySelectorAll('a').forEach((link) => {
                ...
                if (href.includes('app.lottusfinance.com.br') || label.includes('entrar') || label.includes('criar conta')) {
                    link.setAttribute('href', '#');
                    link.classList.add('coming-soon');
                    link.setAttribute('aria-disabled', 'true');
                    link.addEventListener('click', (event) => event.preventDefault());
                    if (label.includes('entrar')) {
                        link.textContent = 'App em breve';
                    } else if (label.includes('criar')) {
                        link.textContent = 'Cadastro em breve';
                    }
                }
            });
        }
